...
Status LoadSavedModelInternal(
    const SessionOptions& session_options, const RunOptions& run_options,
    const std::pair<const void*, size_t>& binary_model,
    const std::unordered_set<string>& tags, SavedModelBundle* const bundle) {
  TF_RETURN_IF_ERROR(ReadMetaGraphDefFromSavedModel(binary_model, tags,
                                                    &bundle->meta_graph_def));
  // I decided that we don't care about the debug info --glarik
  // TF_RETURN_IF_ERROR(
  //    ReadSavedModelDebugInfoIfPresent(binary_model, &bundle->debug_info));
  TF_RETURN_IF_ERROR(LoadMetagraphIntoSession(
      session_options, bundle->meta_graph_def, &bundle->session));
  // RestoreSession calls RunRestore then GetInitOp then RunInitOp then GetCell
  // - RunRestore: loads the variables (or maybe just saves the file paths into
  // string tensors)
  //    - constructs the paths to the variable files and stores them in tensors
  //    - then calls RunOnce with the variable tensors, no outputs, and
  //    restore_op_name from the meta_graph
  //        - iterates thru the inputs (pairs of string/Tensor) - variables
  //            - does add_feed (???)
  //            - then pushes the tensor onto the feed_tensors vector
  // - then runs GetInitOp which only uses export_dir for error message
  // (modified)
  // - then RunInitOp: might be optional
  //    - then calls AddAssetsTensorsToInputs - i think this is mostly optional
  //    as well
  //                                            except for some special models
  //            - this calls CreateStringTensor in a loop with the asset
  //            filename for all assets ( i dont think this actually loads the
  //            the files )
...
Status RestoreSession(const RunOptions& run_options,
  ...
  // const uint64 graph_init_start_microseconds = Env::Default()->NowMicros();
  string init_op_name;
  TF_RETURN_IF_ERROR(internal::GetInitOp(meta_graph, &init_op_name));
  TF_RETURN_IF_ERROR(RunInitOp(run_options, meta_graph, asset_file_defs,
                               session->get(), init_op_name));
  // load_latency_by_stage->GetCell(export_dir, "restore_graph")
  //    ->Add(restore_graph_walltime);
  //// Record wall time spent in init op.
  // load_latency_by_stage->GetCell(export_dir, "init_graph")
